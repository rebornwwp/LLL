...
plotChart title quotes fname = do
  ...
  where
    ...
    (candles, closings, volumes) =
      unzip3 $
      [ (Candle day low open 0 close high, (day, close), (day, [volume]))
      | QuoteData {..} <- toList quotes
      ]
    chart =
      slayouts_layouts .~
      [StackedLayout candlesLayout, StackedLayout volumesLayout] $
      def
    candlesLayout =
      layout_title .~ title $
      layout_plots .~
    ...
    candle label values color =
      ...
      plot_candle_width .~ 3 $
      plot_candle_values .~ values $ plot_candle_title .~ label $ def
    qline label values color =
      plot_lines_style .~ lineStyle 1 color $
      plot_lines_values .~ [values] $ plot_lines_title .~ label $ def
    ...
    lineStyle n color = line_width .~ n $ line_color .~ opaque color $ def
