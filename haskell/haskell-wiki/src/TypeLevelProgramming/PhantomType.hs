...
-- Mile and Kilometer 没有data constructor，这样这两个参数在value level层面是phantom的存在
...
instance UnitName unit => UnitName (Distance unit) where
...
instance UnitName Danwei where
  unitName _ = "Danwei"
...
instance UnitNameAlt u => Show (Distance u) where
