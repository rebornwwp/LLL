...
"WUB" before the first word of the song (the number may be zero), after
the last word (the number may be zero), and between words (at least one
...
songDecoder, songDecoder' :: String -> String
songDecoder = intercalate " " . filter (not .null) . splitOn "WUB"
